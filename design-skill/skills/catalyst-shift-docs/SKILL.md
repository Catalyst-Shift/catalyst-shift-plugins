...
- **Cap body measure** at ~720px in the dark templates. Keep paragraphs short.

### About the visual system (v2, dark-only)

The templates are **screen-first dark documents** that print to clean white via `@media print`. There is one canonical look — no separate `-light` variants. Visual hallmarks that come pre-built and shouldn't be removed:

- Sticky top nav with the `Catalyst<span class="sh">Shift</span>` lockup and a `live` status pill.
- Hero with a thin grid background, a soft signal-cyan glow, and a **cover-strip** of `For · Module · Owner · Version`.
- Numbered section rails (`§ 01`, `§ 02`…) on the left, body content on the right.
- A **module-frame** (faux product chrome with traffic-light dots) wrapping outcome tables, with status `dot`s (`.positive` cyan, `.signal` violet, `.caution` coral).
- Module IDs (`M.01`, `T.02`, etc.) used as in-text anchors in mono.
